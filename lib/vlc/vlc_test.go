...
func Test_splitByChunks(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := splitByChunks(tt.args.bStr, tt.args.chunkSize); got == nil {
...
func TestBinaryChunks__ToHex(t *testing.T) {
	tests := []struct {
		name string
		bcs  BinaryChunks
		want HexChunks
		...
		{
			name: "base test",
			bcs:  BinaryChunks{"0101111", "1000000"},
			want: HexChunks{"2F", "40"},
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
